...
public:
    ...
    int prepare(std::array<uint8_t, CID_BYTES>& recv_client_cid, std::array<uint8_t, crypto_box_PUBLICKEYBYTES>& recv_client_public_key,
                std::array<uint8_t, crypto_sign_PUBLICKEYBYTES>& recv_client_sign_key, const key_mgr_25519& key_mgr, bool is_precalc_hash)
    {
...
public:
    ...
    bool clear_ctx_by_uid(const uint64_t& this_cif, const std::string& uid, uint64_t& cif)
    {
...
public:
    ...
    std::string user_list_to_str(bool show_status)
    {
        ...
        for (auto it : user_db) {
            auto u = it.second;
            if (show_status) {
                u_list += u.unique_name + ((u.user_status == 1) ? (" (in)\n") : ("\n"));
            }
            else {
...
public:
    ...
    // Simplify the socket send function.
    int simple_send(const struct sockaddr_in& address_info, const uint8_t* msg, size_t n)
    {
        return socket.send_to(address_info, lichat::const_byte_span_t{msg, n});
    }

    // Simplify the socket send function.
    int simple_send(uint8_t header, const struct sockaddr_in& address_info, const uint8_t* msg, size_t n)
    {
    ...
    ssize_t simple_secure_send(const uint8_t header, const uint64_t cif, const uint8_t* raw_msg, size_t raw_n)
    {
        ...
        if ((offset + sid.size() + cif_bytes.size() + raw_n + crypto_aead_aes256gcm_ABYTES) > BUFF_BYTES) {
            buffer.send_bytes = offset;
        ...
        auto res =
            crypto_aead_aes256gcm_encrypt(buffer.send_buffer.data() + offset, reinterpret_cast<unsigned long long*>(&aes_encrypted_len),
                                          reinterpret_cast<const uint8_t*>(buffer.send_aes_buffer.data()), buffer.send_aes_bytes, NULL, 0,
                                          NULL, server_aes_nonce.data(), aes_key.data());
    ...
    ssize_t simple_sign_send(const uint8_t header, const uint64_t cif, const uint8_t* raw_msg, size_t raw_n)
    {
    ...
    size_t broadcasting(const uint8_t* msg, const size_t msg_bytes)
    {
        ...
        unsigned long long signed_len = 0;
        if (crypto_sign(buffer.send_buffer.data() + 1, &signed_len, msg, msg_bytes, server_ssk.data()) != 0)
            return 0;
        ...
        for (auto elem : clients.get_ctx_map()) {
            ...
            if (simple_send(address_info, buffer.send_buffer.data(), buffer.send_bytes) >= 0)
                ++sent_out;
        }
        return sent_out;
    }
    ...
    bool decrypt_recv_0x10raw_bytes(const size_t recved_raw_bytes, uint64_t& ret_cif)
    {
        ...
        auto ret              = (crypto_aead_aes256gcm_decrypt(buffer.recv_aes_buffer.data(), &aes_decrypted_len, NULL, beg + offset,
        ...
        if (aes_decrypted_len <= SID_BYTES)
            return false;
        if (ret) {
            if (std::memcmp(buffer.recv_aes_buffer.data(), sid.data(), sid.size()) == 0) {
                ret_cif = cinfo_hash;
                return true;
            }
            return false;
    ...
    std::string user_list_to_msg()
    {
        ...
        oss << "[SYSTEM_INFO] currently " << stat_par.first << " signed up users, " << stat_par.second << " signed in users. list:\n"
    ...
    {
        size_t expected_bytes = 1 + ERR_CODE_BYTES + crypto_sign_PUBLICKEYBYTES + crypto_sign_BYTES + CID_BYTES + crypto_box_PUBLICKEYBYTES;
        ...
        if (crypto_sign_open(nullptr, &unsign_len, beg + offset, crypto_sign_BYTES + CID_BYTES + crypto_box_PUBLICKEYBYTES,
    ...
    bool send_user_list(const uint64_t& cif)
    {
        ...
        std::vector<uint8_t> ulist_vec(ulist_str.data(), ulist_str.data() + ulist_str.size());
    ...
    bool run_server(void)
    {
        ...
        while (true) {
            ...
            for (size_t i = 0; i < 10; ++i) std::cout << (int)buffer.recv_raw_buffer[i] << ' ';
            if (bytes_recv > 10)
                std::cout << "... ";
            ...
            if (header == 0x00 || header == 0x01) {
                size_t expected_size = 1 + crypto_sign_PUBLICKEYBYTES + crypto_sign_BYTES + CID_BYTES + crypto_box_PUBLICKEYBYTES;
                ...
                if (crypto_sign_open(nullptr, &unsign_len, beg + offset, buffer.recv_raw_bytes - offset, spk.data()) != 0) {
                    simple_send(client_addr, server_ff_failed, sizeof(server_ff_failed));
                    continue;
                }
                offset += crypto_sign_BYTES;
                ...
                if (this_conn == nullptr) {
                    simple_send(client_addr, reinterpret_cast<const uint8_t*>(fatal_error), sizeof(fatal_error));
            ...
            if (header == 0x02) {
                size_t expected_size =
                    1 + CIF_BYTES + crypto_aead_aes256gcm_NPUBBYTES + SID_BYTES + sizeof(ok) + crypto_aead_aes256gcm_ABYTES;
                ...
                aes_dec_len     = 0;
                auto is_aes_ok =
                    ((crypto_aead_aes256gcm_decrypt(buffer.recv_aes_buffer.data(), reinterpret_cast<unsigned long long*>(&aes_dec_len),
                                                    NULL, pos + CIF_BYTES + crypto_aead_aes256gcm_NPUBBYTES,
                                                    SID_BYTES + sizeof(ok) + crypto_aead_aes256gcm_ABYTES, NULL, 0, client_aes_nonce.data(),
                                                    aes_key.data()) == 0) &&
                     (aes_dec_len == SID_BYTES + sizeof(ok)));

                buffer.recv_aes_bytes = aes_dec_len;
                auto is_msg_ok        = ((std::memcmp(buffer.recv_aes_buffer.data(), server_sid.data(), SID_BYTES) == 0 &&
            ...
            //    0x1F + signed(CIF + '!')
            if (header == 0x1F) {
                if (buffer.recv_raw_bytes != HEARTBEAT_BYTES && buffer.recv_raw_bytes != GOODBYE_BYTES)
                    continue;
                std::array<uint8_t, CIF_BYTES> recved_cif_bytes;
                auto                           beg     = buffer.recv_raw_buffer.data();
                auto                           cif_pos = beg + 1 + crypto_sign_BYTES;
                std::copy(cif_pos, cif_pos + CIF_BYTES, recved_cif_bytes.begin());
                ...
                if (crypto_sign_open(nullptr, &unsign_len, beg + 1, buffer.recv_raw_bytes - 1, client_spk.data()))
                ...
                std::string bcast_msg = timestamp + ",[SYSTEM_BCAST]," + (*uname) + " signed out!";
                // std::cout << bcast_msg << std::endl;
                //  Broadcasting to all active users.
                broadcasting(reinterpret_cast<const uint8_t*>(bcast_msg.c_str()), bcast_msg.size());
                continue;
            }
            if (header == 0x10) {
                size_t expected_min_size = 1 + CIF_BYTES + crypto_aead_aes256gcm_NPUBBYTES + SID_BYTES + crypto_aead_aes256gcm_ABYTES;
                ...
                if (stat == 1) {
                    ...
                    if (option == 0x00) { // Signing up.
                        ...
                        auto reg_info = lc_utils::split_buffer(msg_body + 2, msg_size - 2, 0x00, 3);
                        ...
                        simple_secure_send(0x10, cinfo_hash, reinterpret_cast<const uint8_t*>(uinfo_res.c_str()), uinfo_res.size());
                        ...
                        timestamp             = lc_utils::now_time_to_str();
                        std::string bcast_msg = timestamp + ",[SYSTEM_BCAST]," + reg_info[1] + " signed up and signed in!";
                        broadcasting(reinterpret_cast<const uint8_t*>(bcast_msg.c_str()), bcast_msg.size());
                        continue;
                    }
                    ...
                    if (signin_type != 0x00 && signin_type != 0x01)
                        continue;
                    auto signin_info = lc_utils::split_buffer(msg_body + 2, msg_size - 2, 0x00, 2);
                    ...
                    std::string uinfo_res = ":" + uemail + ":" + uname;
                    simple_secure_send(0x10, cinfo_hash, reinterpret_cast<const uint8_t*>(uinfo_res.c_str()), uinfo_res.size());
                    ...
                    if (users.get_bind_cif(0, uemail, prev_cif)) {
                        timestamp = lc_utils::now_time_to_str();
                        simple_secure_send(0x10, prev_cif, reinterpret_cast<const uint8_t*>(s_signout), sizeof(s_signout));
                    ...
                    timestamp             = lc_utils::now_time_to_str();
                    std::string bcast_msg = timestamp + ",[SYSTEM_BCAST]," + uname + " signed in!";
                    broadcasting(reinterpret_cast<const uint8_t*>(bcast_msg.c_str()), bcast_msg.size());
                    continue;
                }
                // stat = 2
                ...
                auto        response_size = timestamp.size() + 1 + sender_uname->size() + 1 + msg_size;

                std::vector<uint8_t> resp(response_size, ',');
                offset = 0;
                std::copy(timestamp.begin(), timestamp.end(), resp.begin());
                offset += timestamp.size() + 1;
                std::copy(sender_uname->begin(), sender_uname->end(), resp.begin() + offset);
