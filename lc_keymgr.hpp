...
public:
    ...
    static int read_key_file (const std::string& file_path, 
        std::vector<uint8_t>& content, const std::streamsize& size) {
    ...
    int load_local_key_files () {
        std::vector<uint8_t> cpk(crypto_box_PUBLICKEYBYTES), 
                             csk(crypto_box_SECRETKEYBYTES);
        std::vector<uint8_t> spk(crypto_sign_PUBLICKEYBYTES), 
                             ssk(crypto_sign_SECRETKEYBYTES);
        ...
        if (res1 == 0 && res2 == 0 && res3 == 0 && res4 == 0) {
            ...
            crypto_box_seal(enc_msg, random_msg, sizeof(random_msg), 
                            cpk.data());
            if (crypto_box_seal_open(dec_msg, enc_msg, sizeof(enc_msg), 
                            cpk.data(), csk.data()) != 0)
                return 1;
            if (std::memcmp(random_msg, dec_msg, sizeof(random_msg)) != 0)
                return 3;

            crypto_sign(sign_msg, &signed_len, random_msg, 
                        sizeof(random_msg), ssk.data());
            if (crypto_sign_open(nullptr, &sign_open_len, sign_msg, 
                sizeof(sign_msg), spk.data()) != 0)
                return 5;
    ...
    int gen_key_save_to_local () {
        ...
        if (!out_cpk.is_open() || !out_csk.is_open() || !out_spk.is_open() ||
            !out_ssk.is_open()) {
        ...
        out_cpk.write(reinterpret_cast<const char *>(gen_box_pk.data()), 
                      gen_box_pk.size());
        out_csk.write(reinterpret_cast<const char *>(gen_box_sk.data()), 
                      gen_box_sk.size());
        out_spk.write(reinterpret_cast<const char *>(gen_sign_pk.data()), 
                      gen_sign_pk.size());
        out_ssk.write(reinterpret_cast<const char *>(gen_sign_sk.data()), 
                      gen_sign_sk.size());
