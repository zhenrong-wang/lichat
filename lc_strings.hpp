...
public:
    ...
    static std::string wstr_to_utf8(const std::wstring& wstr)
    {
        ...
        if (sizeof(wchar_t) == 2)
            ...
            ustr = icu::UnicodeString::fromUTF32(reinterpret_cast<const UChar32*>(wstr.data()), wstr.size());
        std::string utf8_str;
    ...
    static std::wstring utf8_to_wstr(const std::string& utf8_str)
    {
        ...
        if (sizeof(wchar_t) == 2) {
            u_strToWCS(nullptr, 0, &required_size, ustr.getBuffer(), ustr.length(), &uerr);
            if (U_FAILURE(uerr))
                return L"";
            wstr.resize(required_size);
            u_strToWCS(&wstr[0], wstr.size(), nullptr, ustr.getBuffer(), ustr.length(), &uerr);
            if (U_FAILURE(uerr))
                return L"";
            return wstr;
        }
